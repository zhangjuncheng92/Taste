...
public class HttpUtilsAsync {
    ...
    public static void post(String url, JsonObject params, AsyncHttpResponseHandler responseHandler) {
        client.setMaxRetriesAndTimeout(retry, timeout);
        client.post(MApp.getInstance().getApplicationContext(), url, ApiUtil.getInstance().getByteArrayEntity(params), CONTENT_TYPE, responseHandler);
        Logs.i("HttpUtilsAsync", url+ "?" + params.toString());
    }
     ...
     * @param responseHandler
     */
    public static void postSystem(String url, JsonObject params, AsyncHttpResponseHandler responseHandler) {
        client.setMaxRetriesAndTimeout(retry, timeout);
        try {
            byte[] s = new Aes(Constants.AES_KEY_SYSTEM).encrypt(params.toString().getBytes("UTF-8"));
            ByteArrayEntity byteArrayEntity = new ByteArrayEntity(Gzip.gzip(s));
            client.setUserAgent(System.getProperty("http.agent"));
            client.addHeader("cmb-ae", "abxd");
            client.addHeader("cmb-g", "mdzz");
            responseHandler.setTag(true);
            client.post(MApp.getInstance().getApplicationContext(), url, byteArrayEntity, CONTENT_TYPE, responseHandler);
            Logs.i("HttpUtilsAsync", url.toString() + "?" + params);
    ...
    public static void postCommon(String url, JsonObject params, AsyncHttpResponseHandler responseHandler) {
        client.setMaxRetriesAndTimeout(retry, timeout);
        try {
            byte[] s = new Aes(Constants.AES_KEY_COMMON).encrypt(params.toString().getBytes("UTF-8"));
            ByteArrayEntity byteArrayEntity = new ByteArrayEntity(Gzip.gzip(s));
            client.setUserAgent(System.getProperty("http.agent"));
            client.addHeader("cmb-ae", "abxd");
            client.addHeader("cmb-g", "mdzz");
    ...
    public static void postInfinite(String url, RequestParams params, AsyncHttpResponseHandler responseHandler) {
        ...
        if (SharePreferencesUtil.getInstance().isLogin()) {
            ...
            params.put("sourceChannel", Constants.sourceChannel);
        }
        responseHandler.setTag(false);
